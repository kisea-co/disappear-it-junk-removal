...
export async function POST(request: Request) {
  try {
    if (Number(request.headers.get('content-length') || 0) > 25000) return NextResponse.json({error:'Request is too large.'},{status:413});
    ...
    const reward = body.reward === '50' ? '$50 off any load size' : body.reward === '25' ? '$25 off any load size' : 'None';

    if (!name || !phone || !email || !location || !junk) return NextResponse.json({error:'Please complete every required field.'},{status:400});
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return NextResponse.json({error:'Please enter a valid email address.'},{status:400});
    if (!process.env.RESEND_API_KEY) return NextResponse.json({error:'The quote service is being connected. Please call us for now.'},{status:503});

    const rows = [
      ['Name',name],['Phone',phone],['Email',email],['Location / ZIP',location],['Preferred pickup date',date],['Trashketball reward',reward],['What needs to be removed?',junk]
    ];
